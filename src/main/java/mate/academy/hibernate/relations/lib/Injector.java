...
public class Injector {
    ...
    public Object getInstance(Class<?> certainInterface, SessionFactory sessionFactory) {
        Object newInstanceOfClass = null;
        Class<?> clazz = findClassExtendingInterface(certainInterface);
        Object instanceOfCurrentClass = createInstance(clazz, sessionFactory);
        ...
        for (Field field : declaredFields) {
            ...
            if (field.getDeclaredAnnotation(Inject.class) != null) {
                Object classToInject = getInstance(field.getType(), sessionFactory);
                newInstanceOfClass = getNewInstance(clazz, sessionFactory);
        ...
        if (newInstanceOfClass == null) {
            return getNewInstance(clazz, sessionFactory);
        }
    ...
    private Object getNewInstance(Class<?> certainClass, SessionFactory sessionFactory) {
        if (instanceOfClasses.containsKey(certainClass)) {
            return instanceOfClasses.get(certainClass);
        }
        Object newInstance = createInstance(certainClass, sessionFactory);
    ...
    private Object createInstance(Class<?> clazz, SessionFactory sessionFactory) {
        Object newInstance;
        try {
            if (clazz.isAnnotationPresent(Dao.class)) {
                Constructor<?> classConstructor = clazz.getConstructor(SessionFactory.class);
                newInstance = classConstructor.newInstance(sessionFactory);
            } else {
