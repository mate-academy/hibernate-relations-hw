            ...
            if (constructor.isAnnotationPresent(Factory.class)) {
                if (sessionFactory == null) {
                    throw new RuntimeException(System.lineSeparator()
                            + "Injector found constructor with @Factory annotation in class '"
                            + clazz.getName() + "'" + System.lineSeparator()
                            + "But object of a SessionFactory was not added to Injector. "
                            + "Add it to Injector using method addSessionFactory()");
                }
                try {
                    return constructor.newInstance(sessionFactory);
                } catch (Exception e) {
                    throw new RuntimeException("Can't create object object of the class", e);
                }
            }
        }
        try {
