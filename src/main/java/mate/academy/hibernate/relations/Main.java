...
public class Main {
    public static void main(String[] args) {
        ...
        Actor paulWilliamWalker = new Actor("Paul William Walker");
        vinDiesel.setCountry(usa);
        paulWilliamWalker.setCountry(usa);
        ActorService actorService = new ActorServiceImpl(new ActorDaoImpl(sessionFactory));
        actorService.add(vinDiesel);
        actorService.add(paulWilliamWalker);

        Movie fastAndFurious = new Movie("Fast and Furious");
        fastAndFurious.setActors(List.of(vinDiesel,paulWilliamWalker));
