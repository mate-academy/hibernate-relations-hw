...
public class Main {
    public static void main(String[] args) {
        ...
        Actor michelleRodriguez = new Actor("Michelle Rodriguez");
        vinDiesel.setCountry(usa);
        michelleRodriguez.setCountry(usa);
        ActorService actorService = new ActorServiceImpl(actorDao);
        actorService.add(vinDiesel);
        actorService.add(michelleRodriguez);

        MovieDao movieDao = new MovieDaoImpl(sessionFactory);
        Movie fastAndFurious = new Movie("Fast and Furious");
        fastAndFurious.setActors(List.of(vinDiesel, michelleRodriguez));
        MovieService movieService = new MovieServiceImpl(movieDao);
        movieService.add(fastAndFurious);
        System.out.println(movieService.get(fastAndFurious.getId()));

        Movie machete = new Movie("Machete");
        machete.setActors(List.of(michelleRodriguez));
        movieService.add(machete);
    }
}
