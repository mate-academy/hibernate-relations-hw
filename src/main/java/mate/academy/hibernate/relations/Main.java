...
public class Main {
    public static void main(String[] args) {
        ...
        Actor theRock = new Actor("Johnson Dwayne");
        theRock.setCountry(usa);
        actorService.add(theRock);

        Movie fastAndFurious = new Movie("Fast and Furious");
        fastAndFurious.setActors(List.of(vinDiesel, theRock));
        MovieService movieService =
                new MovieServiceImpl(new MovieDaoImpl(sessionFactory));
        movieService.add(fastAndFurious);
        System.out.println(movieService.get(fastAndFurious.getId()));

        Movie movie = movieService.get(fastAndFurious.getId());
