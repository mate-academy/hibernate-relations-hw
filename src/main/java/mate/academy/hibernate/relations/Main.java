...
public class Main {
    public static void main(String[] args) {
        ...
        Country fromDBusa = countryService.get(usa.getId());
        System.out.println("Country from DB: " + fromDBusa);

        Actor vinDiesel = new Actor("Vin Diesel");
        vinDiesel.setCountry(usa);
        Actor bruceWillis = new Actor("Bruce Willis");
        bruceWillis.setCountry(usa);
        ActorDao actorDao = new ActorDaoImpl(sessionFactory);
        ActorService actorService = new ActorServiceImpl(actorDao);
        actorService.add(vinDiesel);
        actorService.add(bruceWillis);

        Movie fastAndFurious = new Movie("Fast and Furious");
        fastAndFurious.setActors(List.of(vinDiesel, bruceWillis));
